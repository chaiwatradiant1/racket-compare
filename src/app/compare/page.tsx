...
// ─────────────────────────────────────────────────────────────
// Radar axes (shared with data layer)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// evaluateRow — determine best/worst indices for a compare row
// ─────────────────────────────────────────────────────────────
...
function evaluateRow(
  ...
  // If any values aren't numeric, skip highlighting
  ...
  // If all values are equal, no best/worst distinction
...
// ─────────────────────────────────────────────────────────────
// getRacketValue — safely extract a value from a racket by key
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// PickerSelect — searchable dropdown picker
// ─────────────────────────────────────────────────────────────
...
function PickerSelect({
  ...
  // Filter: exclude already-used rackets (except the one currently selected in this slot)
...
// ─────────────────────────────────────────────────────────────
// ComparePage — main compare page component
// ─────────────────────────────────────────────────────────────
...
export default function ComparePage() {
  ...
  // Read slot IDs from URL: ?ids=a,b,c
  ...
  // Current selected rackets in each slot
  ...
  // Update URL when a slot changes
  ...
  // Group rows: insert group headers from .group property
                ...
                <tr>
                  ...
                  {selectedRackets.map((r, i) => (
                    <th key={r.id} className="compare-table-head-racket">
                      <span
                        className="compare-table-head-color"
                        style={{
                          background: SERIES_COLORS[i % SERIES_COLORS.length],
                        }}
                      />
                      <span className="compare-table-head-brand">
                        {r.brand}
                      </span>
                      <span className="compare-table-head-model">
                        {r.model}
                      </span>
                    </th>
                  ))}
                  ...
                  <React.Fragment key={si}>
                    {section.group && (
                      <tr className="compare-table-group">
                        <th colSpan={selectedRackets.length + 1}>
                          {section.group}
                        </th>
                      </tr>
                    )}
                    {section.rows.map((row, ri) => (
                      <tr key={`${si}-${ri}`}>
                        ...
                        {row.formatted.map((val, ci) => {
                          const isBest =
                            row.bestIndex !== null && ci === row.bestIndex;
                          const isWorst =
                            row.worstIndex !== null && ci === row.worstIndex;
                          const cellClass = [
