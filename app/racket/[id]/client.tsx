      ...
      <section className="detail-body">
        <div className="detail-radar-col">
          ...
          <ul className="play-list">
            {PLAY_ROWS.map((row) => {
              const key = row.key as keyof typeof racket;
        ...
        <div className="detail-spec-col">
          ...
          <table className="spec-table">
            <tbody>
              {SPEC_ROWS.map((row) => {
                const r = row as { key: string; label: string; format?: (v: unknown) => string };
                const value = (racket as unknown as Record<string, unknown>)[r.key];
                return (
                  <tr key={r.key}>
                    <th scope="row">{r.label}</th>
                    <td>
                      {r.format ? r.format(value) : String(value ?? '')}
                    </td>
                  </tr>
                );
              })}
